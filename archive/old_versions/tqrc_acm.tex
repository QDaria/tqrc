% ACM SIGCONF format - Full comprehensive paper
...
% Theorem environments (ACM style)
\theoremstyle{plain}
...
\keywords{Topological quantum computing, Fibonacci anyons, reservoir computing, echo state property, quantum machine learning, negative results}
...
\midrule
\multicolumn{2}{l}{\textit{Fibonacci Anyon Parameters}} \\
$\tau$ & Fibonacci anyon type \\
$\phi$ & Golden ratio $(1+\sqrt{5})/2 \approx 1.618$ \\
$d_\tau$ & Quantum dimension of $\tau$ (equals $\phi$) \\
$\Fib{k}$ & $k$-th Fibonacci number \\
$n$ & Number of anyons in the system \\
$\mathcal{H}_n$ & Fusion Hilbert space for $n$ anyons \\
$R$ & $R$-matrix (braiding eigenvalues) \\
$F$ & $F$-matrix (basis change) \\
$B_i$ & Braid operator exchanging anyons $i \leftrightarrow i+1$ \\
\midrule
\multicolumn{2}{l}{\textit{Reservoir Computing Parameters}} \\
$\mathbf{x}(t)$ & Reservoir state at time $t$ \\
$\mathbf{u}(t)$ & Input signal at time $t$ \\
$\mathbf{y}(t)$ & Output signal at time $t$ \\
$W_{\text{res}}$ & Reservoir weight matrix \\
$W_{\text{in}}$ & Input weight matrix \\
$W_{\text{out}}$ & Output (readout) weight matrix \\
$\alpha$ & Leak rate \\
$\rho$ & Spectral radius \\
$\beta$ & Ridge regression regularization \\
$N$ & Classical reservoir dimension \\
\midrule
\multicolumn{2}{l}{\textit{TQRC-Specific Parameters}} \\
$|\psi(t)\rangle$ & Quantum reservoir state \\
$U_{\text{res}}$ & Reservoir unitary operator \\
$U_{\text{in}}$ & Input encoding unitary \\
$\gamma$ & Amplitude damping rate \\
$\theta$ & Braiding angle \\
$p_i(t)$ & Probability of state $|i\rangle$ \\
\midrule
\multicolumn{2}{l}{\textit{Performance Metrics}} \\
$\NRMSE$ & Normalized Root Mean Square Error \\
$\VPT$ & Valid Prediction Time \\
$\MC$ & Memory Capacity \\
$\MC_Q$ & Quantum Memory Capacity \\
$\lambda_1$ & Largest Lyapunov exponent \\
$T_\lambda$ & Lyapunov time $(1/\lambda_1)$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Information loss in TQRC readout. Quantum state contains $2(d-1)$ parameters, but measurement extracts only $d-1$,a 50\% loss.}
\label{fig:information-loss}
\end{figure}

The dramatic performance gap between TQRC and ESN demands careful analysis. We identify \textbf{four fundamental causes} for TQRC underperformance:
...
Quantum simulation follows the Xu et al.~\cite{Xu2024} string-net protocol with modifications for reservoir dynamics.
