...
At matched dimension, ESN dramatically outperforms TQRC (Fig.~\ref{fig:root-cause}). We conducted rigorous statistical comparison with $n=30$ independent trials per configuration, using bootstrap confidence intervals (10,000 resamples) and Welch's t-test with Bonferroni correction for multiple comparisons:
...
The performance gap is statistically unambiguous: ESN outperforms TQRC by $66$--$233\times$ on Mackey-Glass ($p < 10^{-65}$, Cohen's $d > 200$, ``very large'' effect). This is not a marginal difference---it represents a fundamental architectural limitation.
