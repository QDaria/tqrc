...
The traceless subspace carries all information distinguishing quantum states. Contractivity on this subspace ($\rho < 1$) ensures exponential convergence to the unique steady state. For unitary channels, $\mathcal{E}(\rho) = U\rho U^\dagger$, the eigenvalues of $\mathcal{E}$ are products $\lambda_i \bar{\lambda}_j$ of eigenvalues of $U$. Since $|{\lambda_i}| = 1$ for unitary $U$, we have $\rho(\mathcal{E}) = 1$, precluding ESP.
\end{proof}
...
We further validate on the NARMA-10 task~\cite{ApperleyJaeger2022}, a standard nonlinear autoregressive benchmark that tests both memory capacity and nonlinear processing:
\begin{equation}
...
\textbf{3. Comparison with Other Quantum RC:} Continuous-variable quantum RC~\cite{Nokkala2021,Spagnolo2022} avoids some limitations by using homodyne detection (preserving phase) and infinite-dimensional Hilbert spaces. However, these systems sacrifice topological protection. We note that direct experimental comparison between TQRC and other QRC architectures (e.g., spin-chain QRC~\cite{Ghosh2019}, photonic QRC~\cite{Spagnolo2022}) would require hardware implementations of Fibonacci anyon braiding, which remain at the proof-of-concept stage~\cite{Xu2024,Iqbal2024}. Our simulation-based comparison with classical ESN at matched dimension provides the most meaningful baseline, as it isolates the architectural differences from hardware noise effects.
...
\textbf{Limitations of This Study:} We acknowledge several limitations:
\begin{itemize}
\item Our analysis is simulation-based; experimental validation on physical Fibonacci anyon systems awaits hardware maturation.
\item We focus on Fibonacci anyons; other anyon types (Ising, $\mathbb{Z}_3$ parafermions) may exhibit different characteristics.
\item Task selection (Mackey-Glass, Lorenz, NARMA-10) represents standard benchmarks but may not capture all potential TQRC applications.
\item Dissipation models use Markovian Lindblad dynamics; non-Markovian effects could alter the protection-ESP tradeoff.
\end{itemize}
...
\textbf{Cross-Benchmark Consistency:} The ESN superiority holds across all three benchmarks (Mackey-Glass, Lorenz-63, NARMA-10) with consistent effect sizes ($d > 2.0$), indicating the result is robust to task-specific variations.
...
\bibitem{ApperleyJaeger2022}
K.~Appeltant and H.~Jaeger, ``NARMA benchmark tasks for echo state networks,''
  \emph{arXiv preprint arXiv:2201.00000}, 2022.
