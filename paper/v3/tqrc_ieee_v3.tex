...
\begin{abstract}
Reservoir computing exploits high-dimensional nonlinear dynamics to solve temporal prediction tasks with minimal training cost. We investigate \textbf{Topological Quantum Reservoir Computing (TQRC)}, which proposes to harness the braiding dynamics of Fibonacci anyons as quantum reservoirs. Our analysis reveals a \textbf{fundamental tension}: the very properties that make topological systems attractive for quantum computation (unitary evolution and information preservation) fundamentally \textit{oppose} the Echo State Property (ESP) required for reservoir computing.

We demonstrate rigorously that \textbf{pure unitary TQRC violates the ESP}: unitary evolution preserves distances in Hilbert space, preventing the fading memory necessary for reservoir computing. On Mackey-Glass time series benchmarks, pure unitary TQRC achieves $\NRMSE \approx 1.0$ (equivalent to random guessing). Introducing controlled dissipation with enhanced complex amplitude readout achieves best $\NRMSE = 0.44$, but at the cost of sacrificing topological protection.
...
These results constitute a \textbf{no-go theorem} for topological quantum reservoir computing on chaotic prediction tasks: topological protection and reservoir computing functionality represent fundamentally competing requirements. Notably, TQRC performs comparably to ESN on memory-focused tasks (NARMA-10), suggesting task-dependent applicability. This finding provides essential guidance for the quantum machine learning community, establishing that topological protection should not be pursued as a route to quantum reservoir computing advantage for chaotic time series prediction.
\end{abstract}
...
\textbf{This work investigates Topological Quantum Reservoir Computing (TQRC)}. Our analysis reveals \textbf{negative results}:
...
\item \textbf{ESN Baseline Comparison:} At equivalent dimension on chaotic prediction (Mackey-Glass, Lorenz-63), classical ESN achieves $\NRMSE = 0.02$ vs.~TQRC $\NRMSE = 0.44$, a \textbf{20$\times$ performance gap} (Sec.~\ref{sec:results}). However, on memory-focused tasks (NARMA-10), TQRC performs comparably.
...
This Holevo bound~\cite{Holevo1973} implies that even with optimal measurement strategies, TQRC with $n$ Fibonacci anyons can extract at most $\log_2 \Fib{n-1}$ bits per readout, far fewer than the $2(\Fib{n-1} - 1)$ real parameters encoding the quantum state. Combined with the probability simplex constraint (Section~\ref{sec:root-cause}), this explains the observed $\sim$50\% information loss.
...
\end{equation}
which is vacuously satisfied but becomes tight for multi-step prediction. More critically, the unitary ESP violation (Theorem~\ref{thm:incompatibility}) implies \emph{infinite memory}, causing past inputs to corrupt current predictions. This explains our experimental observation of NRMSE $= 0.966 \approx 1.0$ (random guessing).
...
\textbf{Key insight}: This visualization demonstrates the unitarity-ESP tension. Unitary dynamics cannot ``forget'' initial conditions, while dissipation sacrifices topological protection to achieve fading memory.}
...
\end{equation}
where $u(t)$ is uniform random input in $[0, 0.5]$. NARMA-10 requires maintaining 10-step memory while computing a nonlinear function, directly testing reservoir capabilities.
...
\begin{table}[h]
\centering
\caption{Reported QRC Performance in Literature}
\label{tab:qrc-comparison}
\begin{tabular}{lccc}
\toprule
Architecture & Task & Metric & Source \\
\midrule
Spin-chain QRC & NARMA-10 & MC $\approx$ 8.5 & \cite{Ghosh2019} \\
CV-QRC (Gaussian) & NARMA-5 & NRMSE $\approx$ 0.15 & \cite{Nokkala2021} \\
Photonic QRC & XOR & Acc.\ 97\% & \cite{Spagnolo2022} \\
108-qubit QRC & Classification & Acc.\ 85\% & \cite{Kornjaca2024} \\
\midrule
\textbf{TQRC (this work)} & Mackey-Glass & NRMSE $= 0.44$ & -- \\
Classical ESN & Mackey-Glass & NRMSE $= 0.02$ & -- \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Anyon Type Specificity:} We focus on Fibonacci anyons due to their universal quantum computational properties. Other anyon types (Ising anyons~\cite{Kitaev2003}, $\mathbb{Z}_3$ parafermions~\cite{Fendley2012}, Majorana-based systems~\cite{Sarma2015}) may exhibit different dynamics-ESP relationships. The unitarity-ESP tension applies generically, but quantitative performance gaps may vary.
...
We have presented an investigation of Topological Quantum Reservoir Computing using Fibonacci anyons, revealing limitations that constrain the viability of this approach (Figs.~\ref{fig:summary-table},~\ref{fig:takeaways}). Our main conclusions are:
...
\end{equation}
where $\lambda_{\text{gap}} > 0$ is the spectral gap of $\mathcal{L}$~\cite{Attal2006}. Thus dissipative dynamics recover the ESP with convergence rate $\lambda_{\text{gap}}$.
\end{theorem}
...
\textbf{Primary Comparison (ESN vs.\ TQRC):} For the primary ESN vs.\ TQRC comparison at $d=13$, we report:
\begin{itemize}
...
This confirms the 20$\times$ performance difference is both statistically significant and practically meaningful. 95\% confidence intervals for all metrics use bootstrap resampling (10,000 iterations, bias-corrected and accelerated).
